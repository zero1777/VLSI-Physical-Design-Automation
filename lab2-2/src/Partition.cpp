...
void Group::UpdateNode(Cell *cell) {
    if (cell->node == nullptr) {
        cout << "[Debug] UpdateNode nullptr error\n";
    }
    RemoveNode(cell);
    InsertNode(cell);
}
...
void FM_Algorithm::RandomPartition() {
    ...
    //     sz += cell->size;
    // }
    int currentSizeA = 0, currentSizeB = input->totalSizeB;
    int group = 0;
    for (auto cell : input->cells) {
        if (CheckIfMeetInitConstraint(currentSizeA, currentSizeB)) {
            group = 1;
        }
        else {
            currentSizeA += cell->size[0];
            currentSizeB -= cell->size[1];
        }
        cell->whichGroup = group;
        vgroup[group]->groupSize += cell->size[group];
    }
    if (!CheckIfMeetInitConstraint(vgroup[0]->groupSize, vgroup[1]->groupSize)) {
        cout << "[Debug] Initial partition didn't meet the constraint\n";
    }
    // if (sz == input->totalSize) cout << "Debug: accepted\n";
    
}

bool FM_Algorithm::CheckIfMeetInitConstraint(int currentSizeA, int currentSizeB) {
    if (abs(currentSizeA - currentSizeB) < (abs(currentSizeA + currentSizeB) / 10.0)) {
        return true;
    }
    return false;
}
...
int FM_Algorithm::Loop(bool firstTime) {
    ...
    cout << "[Debug] Finish Loop initialization\n";
    ...
    while (vgroup[0]->numOfCell > 0 || vgroup[1]->numOfCell > 0) {
        ...
        if (baseCell == nullptr) {
            cout << "[Debug] baseCell nullptr" << endl;
            break;
        } 
    ...
    // cout << "Debug: maxPartialSum = " << maxPartialSum << "\n";
    cout << "[Debug] Finish Loop" << endl;

    return maxPartialSum;
}

void FM_Algorithm::ResetCellNode() {
    for (auto cell : input->cells) {
        if (cell->node != nullptr) {
            vgroup[cell->whichGroup]->RemoveNode(cell);
        }
    }
}
...
bool FM_Algorithm::CheckIfBalanced(Cell *baseCell) {
    ...
    int to = !from;
    int areaFrom = vgroup[from]->groupSize - baseCell->size[from];
    int areaTo = vgroup[to]->groupSize + baseCell->size[to];
    if (abs(areaFrom - areaTo) < (abs(areaFrom + areaTo) / 10.0)) return true;
    return false;
...
int FM_Algorithm::UpdateGain(Cell *baseCell) {
    if (baseCell == nullptr) {
        cout << "[Debug] UpdateGain baseCell nullptr" << endl;
    }
...
void FM_Algorithm::ForwardToMaxPartialSum(int step) {
    for (int i=cellStack.size(); i>step; i--) {
...
void FM_Algorithm::UpdateGroupSize(int from, int to, Cell *cell) {
    vgroup[from]->groupSize -= cell->size[from];
    vgroup[to]->groupSize += cell->size[to];
}
